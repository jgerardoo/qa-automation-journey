/*
===============================================================================
Exercise: Poetry Club Door Policy
Source: https://exercism.org/tracks/javascript/exercises/annalyns-infiltration
===============================================================================

Instructions:
In this exercise, you'll be implementing the quest logic for a new RPG game a friend is developing.
The game's main character is Annalyn, a brave girl with a fierce and loyal pet dog.
Unfortunately, disaster strikes, as her best friend was kidnapped while searching for berries in the forest.
Annalyn will try to find and free her best friend, optionally taking her dog with her on this quest.

After some time spent following her best friend's trail, she finds the camp in which her best friend is imprisoned.
It turns out there are two kidnappers: a mighty knight and a cunning archer.

Having found the kidnappers, Annalyn considers which of the following actions she can engage in:
    - Fast attack: a fast attack can be made if the knight is sleeping, as it takes time for him to get his armor on, so he will be vulnerable.
    - Spy: the group can be spied upon if at least one of them is awake. Otherwise, spying is a waste of time.
    - Signal prisoner: the prisoner can be signalled using bird sounds if the prisoner is awake and the archer is sleeping, as archers are trained in bird signaling so they could intercept the message.
    - Free prisoner: Annalyn can try sneaking into the camp to free the prisoner. This is a risky thing to do and can only succeed in one of two ways:
        - If Annalyn has her pet dog with her she can rescue the prisoner if the archer is asleep.
        - If Annalyn does not have her dog then she can free the prisoner if the prisoner is awake and the knight and archer are both sleeping.

You have four tasks: to implement the logic for determining if the above actions are available based on the state of the three characters found in the forest and whether Annalyn's pet dog is present or not.
Detailed task instructions can be found in the exercise source. The tasks are as follows:
    Task 1 - Check if the "Fast attack" action in possible.
    Task 2 - Check if the "Spy" action in possible
    Task 3 - Check if the "Signal prisoner" action in possible
    Task 4 - Check if the "Free prisoner" action in possible

Now help Annalyn free her best friend!
*/

/**
 * The fast attack is available when the knight is sleeping
 *
 * @param {boolean} knightIsAwake
 *
 * @return {boolean} Whether or not you can execute a fast attack.
 */
const knightIsAwake = false;
function canExecuteFastAttack(knightIsAwake) {
    return !knightIsAwake;
}
// Test the function
const fastAttack = canExecuteFastAttack(knightIsAwake);
console.log(fastAttack);

/**
 * A useful spy captures information, which they can't do if everyone's asleep.
 *
 * @param {boolean} knightIsAwake
 * @param {boolean} archerIsAwake
 * @param {boolean} prisonerIsAwake
 *
 * @returns {boolean} Whether or not you can spy on someone.
 */
const archerIsAwake = false;
const prisonerIsAwake = false;
function canSpy(knightIsAwake, archerIsAwake, prisonerIsAwake) {
    return knightIsAwake || archerIsAwake || prisonerIsAwake;
}
// Test the function
const spy = canSpy(knightIsAwake, archerIsAwake, prisonerIsAwake);
console.log(spy);

/**
 * You'll get caught by the archer if you signal while they're awake.
 *
 * @param {boolean} archerIsAwake
 * @param {boolean} prisonerIsAwake
 *
 * @returns {boolean} Whether or not you can send a signal to the prisoner.
 */
function canSignalPrisoner(archerIsAwake, prisonerIsAwake) {
    return prisonerIsAwake && !archerIsAwake;
}
// Test the function
const signalPrisoner = canSignalPrisoner(archerIsAwake, prisonerIsAwake);
console.log(signalPrisoner);

/**
 * The final stage in the plan: freeing Annalyn's best friend.
 *
 * @param {boolean} knightIsAwake
 * @param {boolean} archerIsAwake
 * @param {boolean} prisonerIsAwake
 * @param {boolean} petDogIsPresent
 *
 * @returns {boolean} Whether or not you can free Annalyn's friend.
 */
const petDogIsPresent = true;
function canFreePrisoner(knightIsAwake,
    archerIsAwake,
    prisonerIsAwake,
    petDogIsPresent
) {
    const canFreePrisonerOne = !archerIsAwake && petDogIsPresent;
    const canFreePrisonerTwo = !knightIsAwake && !archerIsAwake && prisonerIsAwake && !petDogIsPresent;
    return canFreePrisonerOne || canFreePrisonerTwo;
}
//Test the function
const freePrisoner = canFreePrisoner(knightIsAwake,
    archerIsAwake,
    prisonerIsAwake,
    petDogIsPresent
);
console.log(freePrisoner);